...
eykt_nav <-read_csv("2019_EYKT_Nav_Data.csv")
eykt_qc <-read_csv("QC_EYKT_2019.csv")
eykt_species <-read_csv("EYKT_2019_species.csv")

#Plot ROV transects#

ggplot(eykt_nav, aes(ROV_X, ROV_Y)) + geom_point() +
...
eykt_nav %>% mutate(Dive = substr(DIVE_NO, 6, 7)) -> eykt_nav

#Converts dive column from text to numeric format
eykt_nav$Dive <- as.numeric(eykt_nav$Dive)

#Rename column names (Typically from SECONDS to Seconds, but this was already "Seconds")
plyr::rename(eykt_nav, replace = c("Seconds" = "Seconds"))-> eykt_nav

#Join Tables using a "full join"
transect <- full_join(eykt_nav, eykt_qc, by = c("Dive", "Seconds"))
...
jpeg(filename = "Figures/EYKT_rov_transects2019.jpg",
...
pdf("output/2019_EYKT_smoothed_transects.jpg")
...
write.csv(outPut, file = "output/eykt_smooth_transect_output.csv") 
...
write.csv(transect_pred, file = "output/2019_EYKT_smooth_predict.csv") #This file will be used in ArcGIS
...
eykt_transects <- read_csv("output/EYKT_2019_smooth_predict_lengths.csv") #This was created in ArcGIS

transect_summary <- eykt_transects %>% group_by(Dive) %>% 
...
eykt_species <- read_csv("EYKT_2019_species.csv") %>% filter(Species == 145)
view(eykt_species)
...
ye_adult <- eykt_species %>% filter(Stage != "JV")
view(ye_adult)

#Join specimen and transect summary table together
#Columns are renamed to avoid confusion with specimen table
plyr::rename(transect_summary, replace = c("Dive" = "Dive")) -> transect_summary
view(transect_summary)

#Make sure to change the Area for each surveyed area (NSEO = 442, SSEO = 1052, CSEO = 1661, EYKT = 739)
eykt_survey <- full_join(transect_summary, ye_adult, by = "Dive") %>% 
  mutate(mgt_area = "EYKT", Area = 739, distance = abs(`Mid X (mm)` * 0.001))
...
eykt_distance <- eykt_survey %>% select(Year, mgt_area, Area, Dive, transect_length_m, distance) %>%
  mutate(YEAR = replace_na(Year, 2019)) 

plyr::rename(eykt_distance, replace = c("mgt_area" = "Region.Label", "Dive" = "Sample.Label",
                                        "transect_length_m" = "Effort" )) -> eykt_distance

#Data has to be in a data frame in order to work in distance
as.data.frame(eykt_distance) -> eykt_distance


##2019 EYKT DENSITY ANALYSIS####################################################

#View Summary of Data
summary(eykt_distance$distance)

#View Historgram of perpendicular distance from transect line 
hist(eykt_distance$distance, xlab = "Distance (m)")

###*Model 1 - Distance Model Fitting*###########################################
eykt.model1 <- ds(eykt_distance, key = "hn", adjustment = NULL,
                  convert.units = 0.000001)

summary(eykt.model1$ddf)

plot(eykt.model1, nc = 10)



###*Model 2 - Cosine Adjustment*################################################
eykt.model2 <- ds(eykt_distance, key = "hn", adjustment = "cos",
                  convert.units = 0.000001)
...
eykt.model3 <- ds(eykt_distance, key = "hr", adjustment = "cos",
                  convert.units = 0.000001)
...
eykt.model4 <-ds(eykt_distance, key = "hr", adjustment = "herm",
                 convert.units = 0.000001)
...
str(eykt.model1$dht$individuals, max = 1)

eykt.model1$dht$individuals$summary

eykt.model2$dht$individuals$summary

eykt.model3$sht$individuals$summary

eykt.model4$sht$individuals$summary

###Abundance Estimate###########################################################
eykt.model4$dht$individuals$N

###Density Estimate#############################################################
eykt.model4$dht$individuals$D
